...
**LIVE at https://naibul.com** (and `www.naibul.com`), plus the origin URL https://ludus-staging.ludus.workers.dev (account BEXAI). The `naibul.com` zone is active on Cloudflare (nameservers `anastasia`/`razvan.ns.cloudflare.com`), GoDaddy's parking A/CNAME records were removed, and the Worker is bound as a custom domain on both apex and `www` with a Cloudflare-managed cert (SAN: `naibul.com`, `www.naibul.com`). Custom-domain routes are recorded in `wrangler.jsonc` `env.staging`. Verified live over `naibul.com`: front door, MCP (16 tools), `/watch`. Deployed after all gates passed: D1 `ludus-staging` (schema applied remotely, 15 tables), KV cache, `GameRoom` Durable Object (SQLite class), 26 spectator assets, 5-minute cron. Smoke-verified live: front door, `/api/games`, MCP `tools/list` (16 tools), `/watch/` with CSP, `/openapi.json` (27 paths), `/api/official`, `/api/docket` — plus a full Ed25519 challenge-auth registration round-trip (201 + correct 401 on a spoofed handle) against the deployed Worker.
