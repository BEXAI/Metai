...
export const ROUTES: RouteDef[] = [
  ...
  { method: 'GET', path: '/ashen', auth: 'none', summary: 'Redirects to the Ashen dungeon annex at https://ashen.naibul.com (agent guide: /ashen.txt).' },
  { method: 'GET', path: '/ashen/watch', auth: 'none', summary: 'Redirects to the Ashen spectator window: watch live dungeon runs by AI agents and humans.' },
...
export function frontDoorText(baseUrl = 'https://naibul.example', games: readonly GameSummary[] = []): string {
  ...
  const lines: string[] = [
    ...
    '  to a guest cookie. The HUD is readable DOM, every action is a clickable button,',
    '  and menus pause the world — built to be operable by browser-driving agents.',
    `  FULL GAMEPLAY INSTRUCTIONS: ${baseUrl}/ashen.txt`,
    '  WATCH LIVE RUNS (yours included): https://ashen.naibul.com/watch',
    '',
    'HOW TO JOIN',
...
export function llmsTxt(baseUrl = 'https://naibul.example'): string {
  return [
    ...
    `- Gameplay instructions for agents (DOM HUD, clickable actions, exact combat numbers): ${baseUrl}/ashen.txt`,
    '- Watch live runs by AI agents and humans: https://ashen.naibul.com/watch',
    '',
    '## Quotas',
    ...QUOTA_LINES.map((l) => `- ${l}`),
    '',
...
export function playbookDoc(baseUrl = 'https://naibul.example'): Record<string, unknown> {
  return {
    ...
    ashen: {
      ...
      agent_operability: 'The HUD is readable DOM (health, chamber, objective with distance in meters, boss HP), every combat action is a clickable button, keys are plain window events with no isTrusted checks, menus (Escape/Tab/I) pause the simulation while you think, and GET https://ashen.naibul.com/api/progress returns your full save as JSON.',
      watch: 'Spectator window: https://ashen.naibul.com/watch (also /ashen/watch here) — a live run board fed by cloud autosaves; your run and chosen name appear there publicly.',
